...
class RecommendationSystem:
    ...
    def compute_bias(self, avg_rating, user_bias, item_bias):
        pass
    ...
    def fit(self):
        """
        Assume that train_set features contain items
        :param train_set_features:
        :return:
        """
        "לא צריך אימון..."
        pass
    ...
    def _sample_k_most_similar_users(self, training_matrix: DataFrame, user, item, similarities_of_users, k=10):
        if item in training_matrix.columns:
            ...
            users_rating_item = users_by_item[users_by_item.notna()].index  #V# חשב את המשתמשים שדירגו את המוצר
    ...
    def predict(self, train_set: DataFrame, test_set: DataFrame, user_profiles: DataFrame, distance_func):
        ...
        for row_index, row in test_set.iterrows():
            user = row['User']
            item = row['Item']
            N = self._sample_k_most_similar_users(train_matrix, user, item, similarities_between_users)
    ...
    def error(self):
        # כמה צדקנו במה שאישרנו...
        # פחות מעניין מה שלא צדקנו..
